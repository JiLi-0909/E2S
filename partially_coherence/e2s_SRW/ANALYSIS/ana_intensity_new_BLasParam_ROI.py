...
def GetSampleStruct(LastCreatedFile):
    ...
    print(' now in GetSampleStruct function from e2s_SRW/ANALYSIS/ana_intensity_new_BLasParam_ROI...')
    ...
    try:
        ...
        print('FBT: "too many indices for array" error...downgrading the solution to low rank...')
        return 0,100,100
    ...
    binx = (xx[-1]-xx[0])/len(x) # x-bin size MA 7/6/2019
    biny = (yy[-1]-yy[0])/len(y) # y-bin size MA 7/6/2019
    ...
    # 1st pass - MA 7/6/2019 [define a ROI to avoid bad std calculation due to low-extended noise]
    ...
    # redefine ROI  BEGIN (upgrade ajouter par Marco en Juin 2019 ) ---------------------------------------
    ns = 5 # number of STD to define the ROIs <<<<<<< 
    Nx = int(ns*sigma1 / binx) # n. of bins for ROI1
    Ny = int(ns*sigma2 / biny) # n. of bins for ROI2
    ctrx = int(mu1/binx) # in n. of bins
    ctry = int(mu2/biny) # in n. of bins
    # intervals are reduced around the peak according to ns
    # if ns is too large then intervals will correspond to the original window limits
    int1n=range(max(int(len(x)/2)+ctrx-Nx,int1[0]),min(int(len(x)/2)+ctrx+Nx,int1[-1]))
    int2n=range(max(int(len(y)/2)+ctry-Ny,int2[0]),min(int(len(y)/2)+ctry+Ny,int2[-1]))
    xx    = xx[int1n]
    a_x   = a_x[int1n]
    yy    = yy[int2n]
    a_y   = a_y[int2n]
    ...
    # ------------------------------ ROI redefined around the main peak FINISHED ---------------------------
    if figg==1:
