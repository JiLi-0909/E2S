## ShadowTools, calculates the histogram data used in the analysis. 
## ONLY EDIT plotxy!!!
## 
...
## Takes the path and creates a global variable 
def path(path):
	global direct
	direct=path
	return direct

## The module activatly used in the vibration simulation 
def plotxy(rootnumber,beam,col_h,col_v, nofwhm=1,title="", **kwargs):
  """
  JL: modified to read the beamsize for optimiser
...
## Save the results of the Shadow program in txt files
  foldername= 'objects'
  os.system('[ -d '+foldername+' ] && echo "output directory exists ..." || mkdir '+foldername)
  timestamp = "{:%Y-%b-%d_%H:%M:%S}".format(datetime.datetime.now())
  filename= 'nsgaTest_' +str(rootnumber)+'_sample_information_'+ timestamp +  '.txt'
  os.chdir( foldername )
  #tmp_h_b = []
  #tmp_h_h = []
  ...
  #  tmp_v_h.append(v)
  ##print(tmp_h_b)
  #hl = [x for x in range(len(tmp_h_b)) if tmp_h_h[x] >1]#JL need to modify if change the number of rays,50000-2
  #vl = [x for x in range(len(tmp_v_b)) if tmp_v_h[x] >1]
  ##size_h=tmp_h_b[max(hl)]-tmp_h_b[min(hl)]#JL modified for full beam size objectives 
  ##size_v=tmp_v_b[max(vl)]-tmp_v_b[min(vl)]
  #size_h=-1*tmp_h_b[min(hl)]
  #size_v=tmp_v_b[max(vl)]+0.02
  print('JL:now we are going to save beam information')
  print('fwhmh',str(tkt["fwhm_h"]))
  print('fwhmv',str(tkt["fwhm_v"]))
  #print(size_h)
  #print(size_v)

  f = open(filename, 'w')
  f.write(str(tkt["intensity"])+'\n'+str(tkt["fwhm_h"])+'\n'+str(tkt["fwhm_v"]))
  f.close()
  ...
  #outh   = leastsq( errfunc, inith, args=(tmp_h_b, tmp_h_h))
  #outv   = leastsq( errfunc, initv, args=(tmp_v_b, tmp_v_h))



#JL:no need-----------------------------------------------------

  ## Reads in the GUI inputs and makes them numeric 
  #filein=open('Inputs.txt')
  #Inputs=filein.readlines()
  #params=Inputs[1]
  
  #vars=[]
  #for i in range(len(params.split(','))):#
  #	  vars.append(params.split(',')[i].replace('[','').replace(']',''))
  
  #mydir= os.getcwd()
  
# ## FIGURE CREATION (ONLY FOR DEBUGING FAILING CODE)(WILL OUTPUT FOR EVERY RUN, NUMPTS*OE)
...
#jl:no need-----------------------------------------------------
## Fit function for histograms, Gaussian fit used here
  #fitfunc  = lambda p, x: p[0]*numpy.exp(-0.5*((x-p[1])/p[2])**2)+p[3]
  #errfunc  = lambda p, x, y: (y - fitfunc(p, x))
  #inith  = [1e+2, 0.01, 1, 5]
  #initv  = [1e+2, 0.001, 1, 1]
  ##popt, pcov = curve_fit(fitfunc, tmp_h_b, yn)
  #outh   = leastsq( errfunc, inith, args=(tmp_h_b, tmp_h_h))
  #outv   = leastsq( errfunc, initv, args=(tmp_v_b, tmp_v_h))
  #ch = outh[0] 
  #cv = outv[0]
  ...
  ## Size of the beam is the FWHM of the fit 
  #FWHMH=2.35*abs(ch[2]) #Horizontal FWHM
  #FWHMV=2.35*abs(cv[2]) #Vertical FWHM 
    
  ## Position of the beam based on the loction of the peak of the fit 
  #bin_maxh = numpy.where(fitfunc(ch, tmp_h_b) == fitfunc(ch, tmp_h_b).max())
  #bin_max_inth=numpy.asarray(tmp_h_b)[bin_maxh][0]
  #bin_maxv = numpy.where(fitfunc(cv, tmp_v_b) == fitfunc(cv, tmp_v_b).max())
  #bin_max_intv=numpy.asarray(tmp_v_b)[bin_maxv][0]
 
 
 ## Annotation on Figures 
 
 #axHistx.plot(tmp_h_b, fitfunc(ch, tmp_h_b))
 #axHisty.plot(fitfunc(cv,  tmp_v_b),tmp_v_b)
 # if title!=None:
 ...
 # axText.text(0.0,1.1,"Fitted Horizontal FWHM: %8.5f " %FWHMH)
 # axText.text(0.0,0.9,"Fitted Vertical FWHM: %8.5f " %FWHMV)
...
## Save the results of the Shadow program in txt files 
  ...
  #fileBMIH = open('BeamCentrePositionHori.txt', 'a')
  #fileBMIH.write('%s\n' %bin_max_inth)
  #fileBMIH.close()
  
  #fileBMIV = open('BeamCentrePositionVerti.txt', 'a')
  #fileBMIV.write('%s\n' %bin_max_intv)
  #fileBMIV.close()
  
  #fileBH = open('BeamSizeHori.txt', 'a')
  #fileBH.write('%s\n' %FWHMH)
  #fileBH.close()
  
  #fileBV = open('BeamSizeVerti.txt', 'a')
  #fileBV.write('%s\n' %FWHMV)
  #fileBV.close()
  
  ## Find the correct file to save data to 
  #filename = datetime.datetime.now().strftime("%Y%m%d-%H%M")
  #mydir=(max([os.path.join(mydir,d) for d in os.listdir(mydir)], key=os.path.getmtime))
  #mydir=(max([os.path.join(mydir,d) for d in os.listdir(mydir)], key=os.path.getmtime))
  
  ## When the txt files has as many lines are input points move this to the correct directory 
  #count=0
  #with open('targets.txt','rb') as f:
  #   for line in f:
  #       count+=1

  #if count==1:#int(vars[1]):     
  #   os.rename("BeamCentrePositionHori.txt",os.path.join(mydir,"BeamCentrePositionHori_%s.txt" % filename))
  #   os.rename("BeamCentrePositionVerti.txt",os.path.join(mydir,"BeamCentrePositionVerti_%s.txt" % filename))
  #   os.rename("BeamSizeVerti.txt",os.path.join(mydir,"BeamSizeVerti_%s.txt" % filename))
  #   os.rename("BeamSizeHori.txt",os.path.join(mydir,"BeamSizeHori_%s.txt" % filename))	 
  #   os.rename("Intensity.txt",os.path.join(mydir,"Intensity_%s.txt" % filename))
	 
 
 

  ## Save figures if necessary 
  #figurename1 = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
  #figure.savefig("Figures%s.png" % figurename1)
  return tkt
