...
def crossover(pop):
    Q = []
    for n in range(len(pop)//2): #JL modified to fit python3
...
#def execfile(filepath, globals,locals=None):
#    if globals is None:
#        globals = {}
#    globals.update({
#        "__file__": filepath,
...
def load_input(filename):
    ...
    execfile(filename, options)#JL:modified to fit python3
...
def dump_fronts(fronts, generation):
    ...
    INDEX_FORMAT_CODE = '%d'
    f = open('fronts.' + INDEX_FORMAT_CODE  % generation, "w")#jl modified to fit python3
    f.write("fronts = (\n")
