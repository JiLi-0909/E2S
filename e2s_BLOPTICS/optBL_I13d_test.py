...
def optBL(slitDX, slitDY, Ephot):
    ...
    # CRL lens
    # -------- CASE E = 11200 eV
    delta        = 2.71169e-6   #    @11.2keV (SIREPO)
    attenLen     = 0.012545#12542e-6      #[m] @11.2keV (SIREPO) 
    ...
    optM_1 = srwl_opt_setup_surf_height_1d(srwl_uti_read_data_cols(mirror, "\t", 0, 1),'x',_ang=0.002,_amp_coef= 1 ,_size_x= 0.0002,_size_y= 0.0002)
    ...
    optKB_1   = SRWLOptMirEl(_p=30.9, _q=9.18, _ang_graz=grazG, _r_sag=1.e+23,
    ...
    tG, sG  = 0.3, 0.3  
    grazG   = 3e-3 # 1. * np.pi / 180; 
    optKB_2   = SRWLOptMirEl(_p=33.1, _q=6.888, _ang_graz=grazG, _r_sag=1.e+23,
                             _size_tang=tG, _size_sag=sG,
                             _nvx=cos(grazG), _nvy=0, _nvz=-sin(grazG), _tvx=-sin(grazG), _tvy=0)
    ...
    DKBSam        = 6.9-0.8     #  .1:46/40   0:42/38   -.1: 38/36   -.2:34/34     -.3:30/32 
    ...
    propagParPM    =  [0, 0, 1., 0, 0, 1.0, 1.0, 1.0, 1., 0, 0, 0]
    propagParCryst =  [0, 0, 1., 0, 0, 1.0, 1.0, 1.0, 1., 0, 0, 0]
    propagParKB    =  [0, 0, 1., 0, 0, 1.0, 1.0, 1.0, 1., 0, 0, 0]

   #------------------------
    #propagParApert =  [0, 0, 1., 0, 0, 1.2, 1.2, 1.0, 1.0, 0, 0, 0]
    #propagParLens  =  [0, 0, 1., 0, 0, 1.2, 1.2, 1.0, 1.0, 0, 0, 0]
    #propagParDrift =  [0, 0, 1., 1, 0, 1.2, 1.2, 1.0, 1.0, 0, 0, 0]
    #propagParPM    =  [0, 0, 1., 0, 0, 1.2, 1.2, 1.0, 1.0, 0, 0, 0]
    #propagParCryst =  [0, 0, 1., 0, 0, 1.2, 1.2, 1.0, 1.0, 0, 0, 0]
    #propagParKB    =  [0, 0, 1., 0, 0, 1.2, 1.2, 1.0, 1.0, 0, 0, 0]
    

    ####### original from SRW   #######
    #propagParApert =  [0, 0, 1.0, 0, 0, 1.2, 1.0, 1.0, 1.0, 0.0, 0.0, 0.0]
    #propagParLens  =  [0, 0, 1.0, 0, 0, 1.2, 1.0, 1.0, 1.0, 0.0, 0.0, 0.0]
    #propagParDrift =  [0, 0, 1.0, 1, 0, 1.2, 1.0, 1.0, 1.0, 0.0, 0.0, 0.0]
    #propagParPM    =  [0, 0, 1.0, 0, 0, 1.2, 1.0, 1.0, 1.0, 0.0, 0.0, 0.0]
    #propagParCryst =  [0, 0, 1.0, 0, 0, 1.2, 1.0, 1.0, 1.0, 0.0, 0.0, 0.0]
    #propagParKB    =  [0, 0, 1.0, 0, 0, 1.2, 1.0, 1.0, 1.0, 0.0, 0.0, 0.0] 
    ...
    #oe.append( SRWLOptA('r', 'o', 100*1e-6, 100*1e-6) )                                                                                                       # oe(1):  aperture
