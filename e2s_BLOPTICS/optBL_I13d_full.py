...
def optBL(slitDX, slitDY, Ephot):
    ...
    ang_dif_pl = np.pi/2#np.pi + np.pi/2
    defG_2 =  dthet#+bragg_Ang
    orientDataCr2 = optCr_2.find_orient(Ephot+dE_error,  np.pi/2) # (GsnBm.avgPhotEn) 
    ...
    orientDataCr3 = optCr_3.find_orient(Ephot+dE_error,  -np.pi/2) # (GsnBm.avgPhotEn) 
    ...
    orientDataCr4 = optCr_4.find_orient(Ephot+dE_error, -np.pi/2) # (GsnBm.avgPhotEn) 
    ...
    DKBSam        = Par_new1#6.1#6.9-.8     #  .1:46/40   0:42/38   -.1: 38/36   -.2:34/34     -.3:30/32 
...
#-------------------source shifts propagator-------------------------------------------------------
    propagParDrift1 = propagParDrift
    propagParDrift2 = [  0,  0,  1.,  1,    0,   1.08,  1,  1.0,  1.0,  0, 0, 0]
    propagParDrift3 = [  0,  0,  1.,  1,    0,   1.2, 1.0,  1.0,  1.0,  0, 0, 0]#created for source tilt check
    propagParDrift33 = [  0,  0,  1.,  1,    0,   1.5, 3.1,  1.0,  1.5,  0, 0, 0]
...
#---------------------------------------------------------------------------------------------------
    ...
    MONOB   = 0      
    D_4     = 1
    MONOC   = 0  # def 1second two crystals of the mono
    MONOD   = 0
    D_5     = 0
    KB1     = 0    # def 1
    D_KB    = 0
