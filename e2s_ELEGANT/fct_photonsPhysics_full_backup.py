...
def plot_tune_curves(fichier):    
    ...
    ax2.set_xlabel('E_phot (keV)',fontsize=14)
    ax2.set_ylabel('Flux Density (phot/s/mr^2/0.1%B.W.)',fontsize=14)
    plt.show(ax2)
...
def plot_brightness(fichier2): 
    ...
    photonEnergy= [col for col in df if col.startswith('photonEnergy')]   
    ax22=df.plot(kind='scatter',y=Brightness[0],x=photonEnergy[0],color='Red', marker=".",alpha=.5, logy=1,fontsize=13)    
    for bbb in range(1,len(Brightness)):
        print(bbb)
        df.plot(kind='scatter',y=Brightness[bbb],x=photonEnergy[bbb],color='DarkBlue',marker=".",ax=ax22, alpha=.5,logy=1,fontsize=13)
