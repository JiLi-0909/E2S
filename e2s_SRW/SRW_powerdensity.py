...
# v 0.07
#JL created for power density calculation at free space 
#############################################################################
...
Bx_und  = float(dict['Bx_und'])
phBx      =  float(dict['phBx'])
phBy      = float(dict['phBy'])
...
print('| Bx (T)           = '+str(Bx_und))
print('| phBx (rad)           = '+str(phBx))
print('| phBy (rad)           = '+str(phBy))
...
Bx   = Bx_und #Peak Horizontal field [T]
By   = By_und # 0.3545 #Peak Vertical field [T]
phBx = phBx #Initial Phase of the Horizontal field component
phBy = phBy #Initial Phase of the Vertical field component
...
ycID = 0
zcID = 0#-lam_und*Np_und/2-2*lam_und
# Longitudinal Coordinate of Undulator Center wit hrespect to Straight Section Center [m]
# Calculate from a point before center of undulator,half undulator length and two more periods.
...
elecBeam.partStatMom1.z = -lam_und*Np_und/2-2*lam_und#-0.5*undPer*(numPer + 4) #Initial Longitudinal Coordinate (set before the ID)
...
stkP.allocate(1,200,200)  # (19001, 1, 1) #numbers of points vs photon energy, horizontal and vertical positions
...
#powInAp =uti_math.integ_ar_2d(stkP.arS, 1,[stkP.mesh.xStart, stkP.mesh.xFin, stkP.mesh.nx], [stkP.mesh.yStart, stkP.mesh.yFin, stkP.mesh.ny],[-200/1e-3, 200/1e-3, stkP.mesh.nx], [-200/1e-3, 200/1e-3, stkP.mesh.ny])*1.e+06
