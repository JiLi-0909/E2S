...
OPTICS =  dirname(CWD)+'/e2s_BLOPTICS' # MA 08/11/2020 (one dir up then add SRWLIB)
...
strIntOutFileName2 =  'single-e_'+outfil+'__'+lattice+'_'+timestamp+'.dat' 
#the old 'ex08_res_int2.dat' #file name for output SR intensity data
strIntOutFileName3 = 'multi-e_'+outfil+'__'+lattice+'_'+timestamp+'.dat' 
...
sampFactNxNyForProp = 0.5# 0.25*2 #sampling factor for adjusting nx, ny (effective if > 0)
...
if(srwl_uti_proc_is_master()):
    ...
    #------------------JL:save and extract wavefront part--------------------------------------------------------

    #print('1) Performing Initial Electric Field calculation ... ', end='')
    #arPrecPar[6] = sampFactNxNyForProp #sampling factor for adjusting nx, ny (effective if > 0)
    #srwl.CalcElecFieldSR(wfr2, 0, magFldCnt, arPrecPar)
    #print('2) Simulating Electric Field Wavefront Propagation ... ', end='')
    #print('.\n-----------------JL:saving first wavefront intensity---------------------------------------')
    	
    #wfrnew = deepcopy(wfr2)
    #f = open('/dls/physics/students/sug89938/E2S/e2s_SRW/wfr_DII_I20s_5pm_10msource.txt','wb')
    ...
    ##wfr2 = load_variable('/dls/physics/students/sug89938/E2S/e2s_SRW/wfrnew.txt')
    #wfr2 = load_variable('/dls/physics/students/sug89938/E2S/e2s_SRW/wfr_DII_I20s_50pm.txt')
