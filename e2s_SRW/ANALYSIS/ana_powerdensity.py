...
A=np.array(A)
###################################################
Aresc = A# *1000*1.602*1e-19 # /np.max(A)

#######################integrate total power from intensties##################

powTot =uti_math.integ_ar_2d(Z, 1,[-4e-3, 4e-3,200], [-4e-3, 4e-3,200])*1.e+06 
powTot= powTot*1000*1.602*1e-19
print(powTot)
...
########peakpower = np.max(a_y)
#peakpower = peakpower*1000*1.602*1e-19
...
textstr = '$Peakpower=%.2e$ \n$(w/mm^2)$ \nFWHM(x)=%.2f ($\mu$m) \nFWHM(y)=%.2f ($\mu$m)' % (peakpower, fx, fy) # 
###print(textstr)
