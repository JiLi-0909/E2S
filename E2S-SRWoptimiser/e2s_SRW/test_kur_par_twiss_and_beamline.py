...
#       tt.append(kur(p)) #--> rejeter a la fin
    ...
    for p,task in zip(population,task_range):
        ...
        #f2.write('mpiexec /dls_sw/apps/python/anaconda/1.7.0/64/bin/python kur_fct_for_qsub.py'+' '+ run_name +'\n') # here on voit que kur_fct_for_qsub.py is called HIGH QUEUE#JL : AP cluster
        f2.write('mpirun -np 1 /dls_sw/apps/python/anaconda/1.7.0/64/bin/python kur_fct_for_qsub.py'+' '+ run_name +'\n') # here on voit que kur_fct_for_qsub.py is called HIGH QUEUE#JL: hamilton cluster
        f2.close()
...
#                f17.write
    ...
    with open('/dls/physics/students/sug89938/E2S_JL/E2S-SRWopimiser/e2s_SRW/log_machine.txt','a') as f92:
       f92.write('\n\npopulation:')
