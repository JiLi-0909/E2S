...
sys.path.insert(0, '/dls/physics/mfc33124/03-BLOptimiser/e2s_BLOPTICS/')
...
#import datetime
#SRWLIB      = '/dls/physics/xph53246/source_to_beamline/SRWLIB/' # MA 12/03/2018 - repository created for pure SRWlib files 
##SRWLIB='/dls/physics/mfc33124/SRW_lab_OPT_DESIGN/gitdir/ochubar/SRW/env/work/srw_python/'
#import sys
#sys.path.insert(0, SRWLIB)
#from srwlib import *
#import numpy as np
...
def CalcIntensity(currentOptBL,SRWInputFile,rootnumber):
    ...
    sampFactNxNyForProp = 0# 0 is from Ji0.5  # 0.25*2 #sampling factor for adjusting nx, ny (effective if > 0)
    ...
    # original wfr2.allocate(1, 101, 101) #Numbers of points vs Photon Energy, Horizontal and Vertical Positions
    ...
    wfr2.mesh.nx     =  60 #600   from Ji's low resolution calculation
    wfr2.mesh.ny     =  60 #600
    ...
    if(srwl_uti_proc_is_master()):
        ...
        def load_variable(filename):
            f=open(filename,'rb')
            wfr2=pickle.load(f) # requires "import pickle"
            f.close()
            return wfr2
        ...
        wfr2 = load_variable('/dls/physics/students/sug89938/wfrnew.txt')
